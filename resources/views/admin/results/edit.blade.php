...
<main class="app-main">
    ...
    <div class="app-content">
        <div class="container-fluid">
            <form action="{{ route('admin.results.update', ['id' => $result->id]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="card">
                    <div class="card-body">
                        ...
                        <a href="{{ route('admin.results') }}" class="btn btn-danger" style="margin-top: 20px;">Cancel</a>
                    </div>
                </div>
            </form>
        </div>
...
<script>
    $(document).ready(function () {
        ...
        function loadSubjects(examId, existingSubjects = [], obtainedMarks = [], totalMarks = [], percentage = [], grade = []) {
            if (examId) {
                $.getJSON('/admin/get-subjects/' + examId, function (data) {
