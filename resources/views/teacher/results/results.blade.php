                                    ...
                                    @else
                                        @foreach ($results as $result)
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="{{ route('teacher.results.edit', $result->id) }}" class="btn btn-primary btn-sm">
                                                        <i class="fas fa-pencil-alt"></i>
                                                    </a>
                                                    <form action="{{ route('teacher.results.destroy', $result->id) }}" method="POST" style="display:inline;">
                                                        @csrf
                                                        @method('DELETE')
...
<script>
    function showLoadingAndRedirect() {
        ...
        setTimeout(function () {
            window.location.href = "{{ route('teacher.results.create') }}"; // Redirect to the "Add New Class" page
    ...
    $(document).ready(function () {
        ...
        $('#classes, #sections').on('change', function () {
            ...
            if (classId && sectionId) {
                ...
                $.ajax({
                    url: '{{ route("teacher.results.filter") }}',
                    type: 'GET',
                            ...
                            $.each(data, function (index, result) {
                                let row = `
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('teacher.results.edit', '') }}/${result.id}" class="btn btn-primary btn-sm"><i class="fas fa-pencil-alt"></i></a>
                                            <form action="{{ route('teacher.results.destroy', '') }}/${result.id}" method="POST" style="display:inline;">
                                                @csrf
                                                @method('DELETE')
